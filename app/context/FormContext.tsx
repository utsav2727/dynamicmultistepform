...
export function FormProvider({ children }: { children: React.ReactNode }) {
  ...
  const nextStep = (selectedValue?: string) => {
    ...
    if (currentStep.conditionalRoutes) {
      const selectedOption = selectedValue || formData[currentStep.conditionalRoutes.field];

      if (selectedOption && currentStep.conditionalRoutes.map[selectedOption] !== undefined) {
  ...
  return (
    <FormContext.Provider value={{ stepIndex, steps, formData, nextStep, prevStep, updateFormData, submitForm }}>
      {children}
